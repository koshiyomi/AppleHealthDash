...
### Option 2: Pair with your own iOS HealthKit app
- Use the sample SwiftUI app in `ios/` to build a private bridge that reads HealthKit data with user consent and sends only aggregated metrics to your API.
- The app keeps tokens in memory (no persistence), requests a minimal set of HealthKit types, and posts metrics like steps, resting heart rate, and sleep duration.
- Point both the app and the CLI at the same authenticated endpoint to complete the loop.
